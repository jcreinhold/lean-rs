...
fn planned_builders(
    worker_binary: &Path,
    workload: &Workload,
) -> Result<Vec<lean_rs_worker::LeanWorkerCapabilityBuilder>, Box<dyn std::error::Error>> {
...
fn first_builder(
    worker_binary: &Path,
    workload: &Workload,
) -> Result<lean_rs_worker::LeanWorkerCapabilityBuilder, Box<dyn std::error::Error>> {
