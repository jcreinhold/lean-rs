...
fn baked_toolchain_is_in_the_supported_window() {
    ...
    assert!(
        fp.is_supported(),
        "build-time toolchain {} is not in lean_rs_abi::SUPPORTED_TOOLCHAINS \
         (resolved: {}, digest: {})",
        fp.lean_version,
        fp.resolved_version,
        fp.header_sha256,
    ...
    let entry = lean_rs_abi::supported_for(fp.lean_version).unwrap_or_else(|| {
        panic!(
...
fn resolved_version_aliases_into_the_matched_entry() {
    let fp = lean_toolchain::ToolchainFingerprint::current();
    let entry = lean_rs_abi::supported_for(fp.lean_version).expect("baked version must be supported");
    assert!(
...
fn required_symbols_are_present_across_the_window() {
    ...
    for &symbol in lean_rs_abi::REQUIRED_SYMBOLS {
        assert!(
            lean_rs_abi::symbol_in_all(symbol),
...
fn unknown_versions_are_not_supported() {
    assert!(lean_rs_abi::supported_for("0.0.0").is_none());
    assert!(lean_rs_abi::supported_for("4.999.999").is_none());
    assert!(lean_rs_abi::supported_by_digest("deadbeef").is_none());
}
