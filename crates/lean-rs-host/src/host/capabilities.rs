...
//!   `lean-rs-host-shims` Lake package. It contains the 13 mandatory +
//!   3 optional `lean_rs_host_*` `@[export]` symbols that every typed
...
pub struct LeanCapabilities<'lean, 'h> {
    ...
    /// Shim dylib carrying the 13+3 `lean_rs_host_*` `@[export]`
...
impl<'lean, 'h> LeanCapabilities<'lean, 'h> {
    ...
    /// session-dispatch symbol addresses from the **shim** dylib: 13
    /// mandatory baseline symbols (load failure on miss) and 3
    ...
    /// initializer or any of the 13 **mandatory** symbols is missing
    ...
    pub(crate) fn new(
        ...
        // The 13 mandatory + 3 optional `lean_rs_host_*` symbols live
    ...
    /// fixed contract (the 13+3 `lean_rs_host_*` symbols pre-resolved
