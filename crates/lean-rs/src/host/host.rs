...
//!
//! A [`LeanHost`] binds a [`crate::LeanRuntime`] borrow to a Lake project
...
//! per-call `dlsym`.
//!
//! See `docs/architecture/03-host-api.md` for the full classification
...
impl<'lean> LeanHost<'lean> {
    ...
    /// [`crate::HostStage::Load`] if `path` does not exist or is not a
    /// directory.
    ...
    ///
    /// Returns [`crate::LeanError::Host`] with stage
    /// [`crate::HostStage::Load`] if the dylib does not exist or fails
    /// to open, [`crate::HostStage::Link`] if the initializer symbol or
    /// any of the seven required session symbols is missing.
