#!/usr/bin/env bash
# Charter guard (AGENTS.md): raw lean_* externs and the unsafe_code opt-out live only in
...
if { printf '%s' "$body" | grep -Eq 'extern[[:space:]]+"C"' && printf '%s' "$body" | grep -q 'lean_'; } ||
	...
	echo "lean-rs charter (AGENTS.md): raw lean_* externs and the unsafe_code opt-out belong only in crates/lean-rs-sys. Declare the symbol there (extend the extern block + REQUIRED_SYMBOLS) and call it through the safe layer." >&2
	exit 2
fi
