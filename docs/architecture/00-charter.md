...
- **`lean-rs-host`** (published, **L2**). The opinionated theorem-prover-host stack built on `lean-rs`: `LeanHost`, `LeanCapabilities`, `LeanSession`, elaboration / evidence / meta surfaces, `SessionPool`. Owns the 16 + 4 `lean_rs_host_*` `@[export]` Lean shim contract its capability dylibs must satisfy. Batch and session-pool operations are methods on `LeanSession` rather than a separate `batch` module. Downstreams that want this opinion add it on top of `lean-rs`; downstreams that don't aren't paying for it.
