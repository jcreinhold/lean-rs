...
| Restart generation | `LeanWorkerLifecycleSnapshot::worker_generation`; worker and pool snapshots | Generation equals supervisor restart count and is observable in lifecycle/resource facts | No per-frame request id or generation field on protocol output | `lean-rs-worker-parent` | Compare snapshots around operations when supervising higher-level sessions | Prompt 32 |
